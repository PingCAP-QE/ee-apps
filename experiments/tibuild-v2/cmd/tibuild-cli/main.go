...
func main() {
	...
	)
	{
		...
		if addr == "" {
			...
			default:
				fmt.Fprintf(os.Stderr, "invalid host argument: %q (valid hosts: development|product)\n", *hostF)
				os.Exit(1)
			}
		}
		timeout = *timeoutF
...
func usage() {
	fmt.Fprintf(os.Stderr, `%s is a command line client for the tibuild API.

Usage:
    %s [-host HOST][-url URL][-timeout SECONDS][-verbose|-v] SERVICE ENDPOINT [flags]

    -host HOST:  server host (development). valid values: development, product
