...
// Server lists the gcs service endpoint HTTP handlers.
type Server struct {
	Mounts         []*MountPoint
...
// New instantiates HTTP handlers for all the gcs service endpoints using the
...
func (s *Server) Service() string { return "gcs" }
...
func (s *Server) MethodNames() []string { return gcs.MethodNames[:] }
...
// MountDownloadObjectHandler configures the mux to serve the "gcs" service
...
func MountDownloadObjectHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/gcs-obj/{bucket}/{*key}", f)
}

// NewDownloadObjectHandler creates a HTTP handler which loads the HTTP request
// and calls the "gcs" service "download-object" endpoint.
func NewDownloadObjectHandler(
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		o := res.(*gcs.DownloadObjectResponseData)
		defer o.Body.Close()
