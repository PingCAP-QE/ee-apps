...
//
// gcs HTTP client encoders and decoders
//
// Command:
...
// and path set to call the "gcs" service "download-object" endpoint
func (c *Client) BuildDownloadObjectRequest(ctx context.Context, v any) (*http.Request, error) {
	...
	{
		p, ok := v.(*gcs.DownloadObjectPayload)
		if !ok {
			return nil, goahttp.ErrInvalidType("gcs", "download-object", "*gcs.DownloadObjectPayload", v)
		}
		bucket = p.Bucket
		key = p.Key
	}
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: DownloadObjectGcsPath(bucket, key)}
	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("gcs", "download-object", u.String(), err)
	}
	if ctx != nil {
...
// gcs download-object endpoint. restoreBody controls whether the response body
	...
	return func(resp *http.Response) (any, error) {
		...
		case http.StatusOK:
			...
			)
			{
				...
				if lengthRaw == "" {
					return nil, goahttp.ErrValidationError("gcs", "download-object", goa.MissingFieldError("length", "header"))
				}
			...
			if err != nil {
				return nil, goahttp.ErrValidationError("gcs", "download-object", err)
			}
			res := NewDownloadObjectResultOK(length, contentDisposition)
			return res, nil
		default:
			body, _ := io.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("gcs", "download-object", resp.StatusCode, string(body))
		}
	}
}

// BuildHeadObjectRequest instantiates a HTTP request object with method and
// path set to call the "gcs" service "head-object" endpoint
func (c *Client) BuildHeadObjectRequest(ctx context.Context, v any) (*http.Request, error) {
	...
	{
		p, ok := v.(*gcs.HeadObjectPayload)
		if !ok {
			return nil, goahttp.ErrInvalidType("gcs", "head-object", "*gcs.HeadObjectPayload", v)
		}
		bucket = p.Bucket
		key = p.Key
	}
	u := &url.URL{Scheme: c.scheme, Host: c.host, Path: HeadObjectGcsPath(bucket, key)}
	req, err := http.NewRequest("HEAD", u.String(), nil)
	if err != nil {
		return nil, goahttp.ErrInvalidURL("gcs", "head-object", u.String(), err)
	}
	if ctx != nil {
		req = req.WithContext(ctx)
	}
...
// DecodeHeadObjectResponse returns a decoder for responses returned by the gcs
	...
	return func(resp *http.Response) (any, error) {
		...
		case http.StatusOK:
			...
			)
			{
				lengthRaw := resp.Header.Get("Content-Length")
				if lengthRaw == "" {
					return nil, goahttp.ErrValidationError("gcs", "head-object", goa.MissingFieldError("length", "header"))
				}
				v, err2 := strconv.ParseInt(lengthRaw, 10, 64)
				if err2 != nil {
					err = goa.MergeErrors(err, goa.InvalidFieldTypeError("length", lengthRaw, "integer"))
			...
			contentDisposition = contentDispositionRaw
			if err != nil {
				return nil, goahttp.ErrValidationError("gcs", "head-object", err)
			}
			res := NewHeadObjectResultOK(length, contentDisposition)
			return res, nil
		default:
			body, _ := io.ReadAll(resp.Body)
			return nil, goahttp.ErrInvalidResponse("gcs", "head-object", resp.StatusCode, string(body))
		}
	}
}
