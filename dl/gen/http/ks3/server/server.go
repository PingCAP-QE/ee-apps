...
// New instantiates HTTP handlers for all the ks3 service endpoints using the
...
func New(
	...
	return &Server{
		Mounts: []*MountPoint{
			{"DownloadObject", "GET", "/s3-obj/{bucket}/{*key}"},
			{"HeadObject", "HEAD", "/s3-obj/{bucket}/{*key}"},
		},
...
func (s *Server) Service() string { return "ks3" }
...
func (s *Server) MethodNames() []string { return ks3.MethodNames[:] }
...
// MountDownloadObjectHandler configures the mux to serve the "ks3" service
...
func MountDownloadObjectHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/s3-obj/{bucket}/{*key}", f)
}

// NewDownloadObjectHandler creates a HTTP handler which loads the HTTP request
// and calls the "ks3" service "download-object" endpoint.
func NewDownloadObjectHandler(
	...
	var (
		decodeRequest  = DecodeDownloadObjectRequest(mux, decoder)
		encodeResponse = EncodeDownloadObjectResponse(encoder)
		encodeError    = goahttp.ErrorEncoder(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "download-object")
		ctx = context.WithValue(ctx, goa.ServiceKey, "ks3")
		...
		o := res.(*ks3.DownloadObjectResponseData)
		defer o.Body.Close()
...
// MountHeadObjectHandler configures the mux to serve the "ks3" service
...
func MountHeadObjectHandler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("HEAD", "/s3-obj/{bucket}/{*key}", f)
}

// NewHeadObjectHandler creates a HTTP handler which loads the HTTP request and
// calls the "ks3" service "head-object" endpoint.
