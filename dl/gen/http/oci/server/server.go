...
// MountDownloadFileSha256Handler configures the mux to serve the "oci" service
...
func MountDownloadFileSha256Handler(mux goahttp.Muxer, h http.Handler) {
	...
	mux.Handle("GET", "/oci-file-sha256/{*repository}", f)
}

// NewDownloadFileSha256Handler creates a HTTP handler which loads the HTTP
// request and calls the "oci" service "download-file-sha256" endpoint.
func NewDownloadFileSha256Handler(
	...
	var (
		decodeRequest  = DecodeDownloadFileSha256Request(mux, decoder)
		encodeResponse = EncodeDownloadFileSha256Response(encoder)
		encodeError    = goahttp.ErrorEncoder(encoder, formatter)
	)
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		ctx := context.WithValue(r.Context(), goahttp.AcceptTypeKey, r.Header.Get("Accept"))
		ctx = context.WithValue(ctx, goa.MethodKey, "download-file-sha256")
		ctx = context.WithValue(ctx, goa.ServiceKey, "oci")
		payload, err := decodeRequest(r)
		if err != nil {
			if err := encodeError(ctx, w, err); err != nil && errhandler != nil {
		...
		o := res.(*oci.DownloadFileSha256ResponseData)
		defer o.Body.Close()
		if wt, ok := o.Body.(io.WriterTo); ok {
			if err := encodeResponse(ctx, w, o.Result); err != nil {
				if errhandler != nil {
